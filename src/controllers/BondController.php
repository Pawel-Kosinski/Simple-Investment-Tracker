...
class BondController extends AppController {
    ...
    /**
     * Parsuje kod obligacji i zwraca informacje o niej
     * Przykład: EDO0935 -> typ: EDO, wykup: 09/2035
     */
    private function parseBondCode(string $bondCode): ?array
    {
        // Usuń spacje i konwertuj na uppercase
        $bondCode = strtoupper(trim($bondCode));
        
        // Wzorzec: 3-4 litery + 4 cyfry (MMYY)
        if (!preg_match('/^([A-Z]{3,4})(\d{2})(\d{2})$/', $bondCode, $matches)) {
            return null;
        }
        ...
        // Mapowanie typów
        $validTypes = ['OTS', 'ROR', 'DOR', 'TOS', 'COI', 'EDO', 'ROS', 'ROD'];
        ...
        // Szacowana data emisji (zazwyczaj 10 lat wcześniej dla EDO/ROD, itd.)
        $yearsBack = 0;
        switch ($type) {
            case 'EDO':
            case 'ROD':
        ...
        $issueYear = (int)$year - $yearsBack;
        $issueDate = $issueYear . '-' . $month . '-01';
        
        // UWAGA: Oprocentowanie NIE jest zahardkodowane!
        // Każda emisja ma swoje - użytkownik musi podać z listu emisyjnego
        
        return [
            'type' => $type,
            'maturity_date' => $maturityDate,
            'issue_date' => $issueDate,
            'interest_rate' => null,        // użytkownik wpisze sam
            'first_year_rate' => null,      // użytkownik wpisze sam
            'interest_margin' => null,      // użytkownik wpisze sam
            'rate_base' => $this->getRateBase($type)
        ];
    }
    
    /**
     * Określa bazę oprocentowania dla danego typu obligacji
     */
    private function getRateBase(string $type): ?string
    {
        if (in_array($type, ['COI', 'EDO', 'ROS', 'ROD'])) {
            return 'inflation';
        } elseif (in_array($type, ['ROR', 'DOR'])) {
            return 'nbp_rate';
        }
        return null; // OTS, TOS - stała stopa
    }
